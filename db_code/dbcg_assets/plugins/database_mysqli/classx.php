...
class database_mysqli extends DBCG {
	...
	$this->show_compact_button = 1;
		$return = '$mysqli = mysqli_connect(\'' . $this->myServer . '\', \'' . $this->myUser . '\', \'' . $this->myPass . '\', \'' . $this->myDB . '\') or die(mysqli_error($mysqli));';
	...
	function connect_long() {
		$this->show_compact_button = 1;
			...
			$return .= '$mysqli = mysqli_connect($myServer, $myUser, $myPass, $myDB) or die(mysqli_error($mysqli));';
		return $this->tabulate($return);
	}
	...
	$return='';
	        ...
	        $return .= '<br />[tab]' . $this->db_result('','i');
